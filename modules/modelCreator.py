...
def createModelSmall(splits, x_train, x_val, x_test, batchSize, seed_value, num_of_classes, numOfAttentionLayers, dmodel, header, dff, rate = 0,useEmbed=False):    
        x_trains = np.dsplit(x_train, splits)
        print(np.array(x_trains).shape)
        ...
        for i in range(len(x_trains)):
            ...
            maskLayer = tf.keras.layers.Masking(mask_value=-2)
            encInput = maskLayer(encoded)
            for i in range(numOfAttentionLayers*2):
                ffn = transformer.point_wise_feed_forward_network(dmodel, dff)
                do = tf.keras.layers.Dropout(rate, seed=seed_value)
                encInput = ffn(encInput)
                encInput = do(encInput)
            enc1 = encInput
            flatten1 = Flatten()(enc1)
            flattenArray.append(flatten1)
...
def calcComplexityMetrics(newVal, newTest):
    ...
    #    spectralEntropyVal += ant.spectral_entropy(x, sf=100, method='welch', normalize=True, nperseg = len(x))
    ...
    #    complexityEstimationVal += helper.ce(x)

    #print('calc entropies tests')
    #for x in newTest.squeeze():
    #    spectralEntropyTest += ant.spectral_entropy(x, sf=100, method='welch', normalize=True, nperseg = len(x)) 
    ...
    #    complexityEstimationTest += helper.ce(x)
    ...
    #complexityEstimationVal = complexityEstimationVal / valLen
    ...
    #complexityEstimationTest = complexityEstimationTest / valLen
